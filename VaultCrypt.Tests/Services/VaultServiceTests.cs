...
namespace VaultCrypt.Tests.Services
{
    public class VaultServiceTests
    {
        ...
        void CreateVaultThrowsForNullValues()
        {
            ...
            Assert.Throws<ArgumentNullException>(() => _service.CreateVault(NormalizedPath.From(nonNullValue), nonNullValue, null!, 1));
        }
        ...
        {
            var vaultFile = TestsHelper.CreateVaultFile();

            _service.CreateSessionFromFile(new byte[16], vaultFile);

            Assert.Equal(PasswordHelper.DeriveKey(new byte[16], new byte[32], 1000), _session.KEY);
        ...
        {
            Assert.Throws<ArgumentNullException>(() => _service.CreateSessionFromFile(null!, NormalizedPath.From("VALID")));
        ...
        void RefreshEncryptedFilesListPopulatesFileListAndRefreshses()
        {
            ...
            {
                Assert.Equal(Encoding.UTF8.GetString(vaultInformation.Item2[i].FileName), fileList[i].Value.FileName);
            ...
            File.Delete(vaultInformation.Item1!);
        }
        ...
        void TrimVaultTrimsVaultAndSavesItToAFile()
        {
            ...
            long newVaultSize = new FileInfo((tuple.Item1!.Value[..^4] + "_TRIMMED.vlt")).Length;

            Assert.True(newVaultSize < oldVaultSize);

            File.Delete(tuple.Item1!);
            File.Delete((tuple.Item1!.Value[..^4] + "_TRIMMED.vlt"));
        }
        ...
        void TrimVaultReturnsSameVaultIfThereIsNothingToTrim()
        {
            ...
            Assert.Equal(new FileInfo(tuple.Item1!).Length, new FileInfo((tuple.Item1!.Value[..^4] + "_TRIMMED.vlt")).Length);
